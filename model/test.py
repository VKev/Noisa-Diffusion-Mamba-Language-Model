...
class TestNoisa(nn.Module):
    def __init__(self, vocab_size=50260, embed_dim=1024, num_heads=4, dropout=0.1):
        super(TestNoisa, self).__init__()
        ...
        # self.mamba1 = Mamba2(d_model=embed_dim, d_state=64, d_conv=4, expand=2)
        self.fc_0 = nn.Linear(embed_dim, embed_dim)
        self.selfAttn2 = SelfAttention(
            ScaledDotProductAttention(d_model=embed_dim, d_k=embed_dim, d_v=embed_dim, h=num_heads, dropout=dropout)
        )
        # self.mamba2 = Mamba2(d_model=embed_dim, d_state=64, d_conv=4, expand=2)
